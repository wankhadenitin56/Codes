...
# Set the path
path = "D:\\Project Work\\AFTER COMPRY MODEL REFINMENT\\MODELS RESULTS\\combine_features/1"
...
# Get the ranking of features from the RFE selector
feature_ranking = rfe_selector.ranking_
# Sort features based on their ranking
sorted_indices = np.argsort(feature_ranking)
# Get the names of selected features
selected_features = X_train.columns[sorted_indices]
# Get the rankings of selected features
selected_ranks = feature_ranking[sorted_indices]
# Plot the graph for top features

print("Plot the graph for top features.......")

plt.figure(figsize=(10, 6))
plt.barh(range(num_top_features), selected_ranks[:num_top_features], align='center')
plt.yticks(range(num_top_features), selected_features[:num_top_features])
...
print("Get the indices of the top 20 features using RFE.....")

top_20_feature_indices_rfe = np.where(rfe_selector.support_)[0]

# Save the feature names to a text file
with open(os.path.join(path, 'top_20_feature_names_REF.txt'), 'w') as file:
    for feature_index in top_20_feature_indices_rfe:
        file.write(train_set.drop("target", axis=1).columns[feature_index] + '\n')

# Save the top 20 feature indices selected by RFE to a CSV file
rfe_features_path = os.path.join(path, 'top_20_rfe_features.csv')
pd.DataFrame(top_20_feature_indices_rfe, columns=['FeatureIndex']).to_csv(rfe_features_path, index=False)
...
# Get the indices of the selected features from Boruta and RFE
selected_feature_indices_boruta = np.where(boruta_selector.support_)[0][:20]
selected_feature_indices_rfe = np.where(rfe_selector.support_)[0][:20]

# Combine the selected feature indices from both Boruta and RFE
combined_feature_indices = np.concatenate((top_20_feature_indices_boruta, top_20_feature_indices_rfe))
