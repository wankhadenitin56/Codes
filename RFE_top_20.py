...
# Set the path
path ="D:\\Project Work\\AFTER COMPRY MODEL REFINMENT\\MODELS RESULTS\\RFE_feature\\1"
...
print(selected_feature_indices_RFE.shape)
# Get the indices of the top 20 features using RFE
top_20_feature_indices_RFE  = np.where(rfe_selector.support_)[0]
print(top_20_feature_indices_RFE.shape)

# Get the names of selected features
selected_features = X_train.columns[top_20_feature_indices_RFE]

# Get the rankings of selected features
selected_ranks = rfe_selector.ranking_[top_20_feature_indices_RFE]


# Plot the graph for top features
print("Plot the graph for top features.......")

plt.figure(figsize=(10, 6))
plt.barh(range(num_top_features), selected_ranks, align='center')
plt.yticks(range(num_top_features), selected_features)
...
# Save the total number of selected features by RFE to a text file
total_selected_features_path = os.path.join(path, 'total_selected_features_in_RFE.txt')
with open(total_selected_features_path, 'w') as f:
    f.write("Total selected features in RFE: {}\n".format(total_selected_features_in_RFE))

print("Total selected features in RFE saved to:", total_selected_features_path)
...
# Save the names of all selected features to a text file
with open(os.path.join(path, 'all_selected_feature_names_RFE.txt'), 'w') as file:
    for feature_index in selected_feature_indices_RFE:
...
with open(top_20_selected_features_RFE_path, 'w') as txt_file:
    for index in top_20_feature_indices_RFE:
        txt_file.write(str(index) + '\n')
...
with open(os.path.join(path, 'top_20_feature_names_RFE.txt'), 'w') as file:
    for feature_index in top_20_feature_indices_RFE:
        file.write(train_set.drop("target", axis=1).columns[feature_index] + '\n')
...
X_train_selected = X_train_normalized[:, top_20_feature_indices_RFE ]
X_test_selected = X_test_normalized[:, top_20_feature_indices_RFE ]
