...
# Set the path
#path =r"D:\Project Work\AFTER COMPRY MODEL REFINMENT\MODELS RESULTS\RFE_WITH_SCORE_20_FEATURE\2"
#os.makedirs(path, exist_ok=True)

# Load the dataset
#n = pd.read_csv(r"C:/Users/wankh/OneDrive/Desktop/project work/Working_on/Maindpp4.csv")

# Set the path
path =r"D:\Project Work\AFTER COMPRY MODEL REFINMENT\MODELS RESULTS\RFE_WITH_SCORE_20_FEATURE\Data_with_6899\1"
os.makedirs(path, exist_ok=True)

# Load the dataset
n = pd.read_csv(r"D:\Project Work\AFTER COMPRY MODEL REFINMENT\multitasking model data and models\dpp4_6899_ML.csv")
...
selected_features_df = pd.DataFrame({
    'FeatureName': selected_features_importance,
    'ImportanceScore': selected_importances
})

# Save the DataFrame to a CSV file
selected_features_df.to_csv(os.path.join(path, 'selected_features_importance.csv'), index=False)
...
# Get the indices of all selected features by RFE
selected_feature_indices_RFE = np.where(rfe_selector.support_)[0]

# Save the indices of all selected features by RFE in a TXT file
all_selected_features_RFE_path = os.path.join(path, 'all_selected_features_RFE.txt')
with open(all_selected_features_RFE_path, 'w') as txt_file:
    for index in selected_feature_indices_RFE:
        txt_file.write(str(index) + '\n')

print("All selected feature indices by RFE saved to:", all_selected_features_RFE_path)

# Save only the top 20 selected features
top_20_selected_features_RFE_path = os.path.join(path, 'top_20_selected_features_RFE.txt')
with open(top_20_selected_features_RFE_path, 'w') as txt_file:
    for index in top_20_feature_indices_importance:
        txt_file.write(str(index) + '\n')

# Save the names of the top 20 selected features to a text file
with open(os.path.join(path, 'top_20_feature_names_RFE.txt'), 'w') as file:
    for feature_index in top_20_feature_indices_importance:
        file.write(X_train.columns[feature_index] + '\n')

# Save the top features graph as an image
top_features_graph_path = os.path.join(path, 'top_20_features_RFE_graph.png')
plt.savefig(top_features_graph_path)
plt.show()

# Select the top features for training and testing sets
X_train_selected = X_train_normalized[:, top_20_feature_indices_importance]
X_test_selected = X_test_normalized[:, top_20_feature_indices_importance]
#######################################################################################################
# Define the paths to save the selected features
X_train_selected_path = os.path.join(path, 'X_train_selected.csv')
X_test_selected_path = os.path.join(path, 'X_test_selected.csv')

# Convert the selected features arrays to DataFrames
X_train_selected_df = pd.DataFrame(X_train_selected, columns=selected_features_importance)
X_test_selected_df = pd.DataFrame(X_test_selected, columns=selected_features_importance)

# Save the selected features DataFrames as CSV files
X_train_selected_df.to_csv(X_train_selected_path, index=False)
X_test_selected_df.to_csv(X_test_selected_path, index=False)

print("Selected features saved as CSV files:")
print("X_train_selected saved to:", X_train_selected_path)
print("X_test_selected saved to:", X_test_selected_path)
...
svm_grid_search = GridSearchCV(SVC (), svm_param_grid, cv=10, scoring='accuracy')
...
params_NB = {'var_smoothing': np.logspace(-10, 10, num=50)}
...
# print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Comparision of the models %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")

# models = ["SVM", "Random Forest", "Naive Bayes"]
# train_accuracies = [0.92, 0.98, 0.70]  
# test_accuracies = [0.86, 0.86, 0.75]   
# colors = ["steelblue", "firebrick", "forestgreen"]
# graph = pd.DataFrame({"models": models, "train_accuracies": train_accuracies, "test_accuracies": test_accuracies})

# plt.figure(figsize=(10, 6))
# bar_width = 0.35
# index = range(len(models))

# plt.bar(index, graph["train_accuracies"], bar_width, label='Train', color=colors)
# plt.bar([i + bar_width for i in index], graph["test_accuracies"], bar_width, label='Test', color=colors, hatch='//')
...
# # Save the plot as an image
# # save_path = "D:/Project Work/MAIN/fULL_REF/1"
