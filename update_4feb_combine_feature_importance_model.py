...
# Set the path
path = r"D:\Project Work\AFTER COMPRY MODEL REFINMENT\MODELS RESULTS\CCOMBINE_FEATURES_WITH_IMPORTANCE\1"
...
print(top_20_feature_indices_importance)
# Create a DataFrame to store top 20 features, indices, and scores
top_20_features_rfe_df = pd.DataFrame(columns=['Feature', 'Index', 'Score'])
...
for i, (feature_name, index, score) in enumerate(zip(selected_features_importance, top_20_feature_indices_importance, selected_importances)):
    ...
    top_20_features_rfe_df = top_20_features_rfe_df.append({
...
top_20_features_rfe_df.to_csv(os.path.join(path, 'top_20_features_rfe_importance.csv'), index=False)
...
# Create a DataFrame to store top 20 features, indices, and scores
top_20_features_boruta_df = pd.DataFrame(columns=['Feature', 'Index', 'Score'])
...
for i, index in enumerate(top_20_feature_indices_rf):
    ...
    # Append the information to the DataFrame
    top_20_features_boruta_df = top_20_features_boruta_df.append({
        'Feature': feature_name,
...
# Save the DataFrame to a CSV file or any other desired format
top_20_features_boruta_df.to_csv(os.path.join(path, 'top_20_boruta_features.csv'), index=False)
...
combined_feature_indices = np.unique(np.concatenate((selected_feature_indices_rfe, selected_feature_indices_boruta)))
print(combined_feature_indices.shape)
